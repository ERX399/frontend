...
function LoginForm() {
  ...
  const performRedirect = useCallback((token: string, redirectOverride?: string) => {
    ...
    const safeTarget = target.startsWith('/') && !target.startsWith('//') ? target : '/forum';
  ...
  const loginWithGithub = async () => {
    ...
    try {
      ...
      // ⚠️ 必须无尾斜杠（2x.nz 的 URL 规范形态）。后端 sanitizeRedirect 也不能补尾斜杠：
      // GitHub callback 用 URL hash 回传 token，带尾斜杠的地址会被 server.js 301 到无斜杠，
      // 而 301 不保留 fragment，token 直接丢，表现为「GitHub 登录点完没反应」。
      // 2026-08-01 刚因后端强制加尾斜杠修过一次，别再加回来。
      const oauthReturnUrl = `${window.location.origin}/forum/auth/login`;
...
/**
 * 已登录态下的登录页：不再给一张可二次登录的表单（那会让每登录一次就增生一台设备，
 * 个人中心「登录设备」里无限叠加）。显示「已登录」+ 去往目标的入口。
 */
function AlreadyLoggedIn({ user, redirect }: { user: ForumUser; redirect: string }) {
  const { logout } = useForumAuth();
  // 与 LoginForm.performRedirect 同一套安全规则：只允许站内路径，拒绝 //host、javascript: 等注入
  const safeTarget = redirect.startsWith('/') && !redirect.startsWith('//') ? redirect : '/forum';
  return (
    <main className="container mx-auto max-w-sm px-4 py-12 text-center space-y-5">
      <Icon icon="mdi:check-circle" className="size-12 mx-auto text-green-600 dark:text-green-400" />
      <div>
        <h1 className="text-2xl font-bold mb-1">已登录</h1>
        <p className="text-sm text-muted-foreground">当前账号：{user.username || user.email}</p>
      </div>
      <Link to={safeTarget} className="block">
      ...
      <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-sm text-muted-foreground">
        <Link to="/forum/me" className="hover:text-foreground">个人中心</Link>
        <Link to="/forum" className="hover:text-foreground">返回论坛</Link>
        <button
          type="button"
          className="text-red-600 hover:text-red-500"
          onClick={() => {
            // 不 await：logout 同步清本地态；先等它异步吊销，页面会停在已登录态
            logout();
...
export default function ForumLoginPage() {
  ...
  // GitHub 回调的 token / 错误码落地在本页 URL（query 或 hash）。此时即使已登录也
  // 不能按「已登录」拦截 —— 新 token 要靠 LoginForm 的 effect 处理并跳走。
  const hasCallbackToken = Boolean(
  ...
  if (!loading && user && !hasCallbackToken) {
    return <AlreadyLoggedIn user={user} redirect={redirect} />;
  }
