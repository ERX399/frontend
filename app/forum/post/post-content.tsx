...
export function PostContent({
  ...
  const loadPost = useCallback(async () => {
    ...
    try {
      ...
      applySeo({
        title: p.title,
        description: makeExcerpt(p.excerpt || p.content || '') || `${p.title} —— 来自夏之论坛的帖子。`,
        path: `/forum/post/${id}`,
        ogType: 'article',
      ...
      <section id="comments" className="mt-8 pt-8 border-t border-border scroll-mt-20">
        ...
        {user ? (
          ...
          <div className="mb-6 -mx-4 px-4 py-4 bg-muted/50 text-center text-sm text-muted-foreground sm:mx-0">
            <Link to={`/forum/auth/login?redirect=/forum/post/${id}`} className="text-primary hover:underline">
              登录
            </Link>
            {' '}后即可评论
          </div>
