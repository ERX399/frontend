...
// ─── 数据模型 ─────────────────────────────────────────────────────────────────
// 四行梯队标签从上到下正好读成「从夯到拉」——从最夯到最拉的排名梗。
...
/** 拖拽时触发自动滚动的视口上下边缘带宽（px）。顶部要盖住 56px 的 fixed header */
const EDGE_ZONE = 96;
...
export default function TierPage() {
      ...
      imgs.map(
        (file) =>
          new Promise<TierItem>((resolve) => {
            ...
            const im = new Image();
            im.onload = () =>
              resolve({ id: uid(), src, ratio: im.naturalHeight ? im.naturalWidth / im.naturalHeight : 1 });
  ...
  const moveItem = useCallback(
    (id: string, from: ZoneId, to: ZoneId, index: number) => {
      setBoard((b) => {
        const src = [...b[from]];
  ...
  const exportImage = useCallback(async () => {
    ...
    try {
      ...
      const totalH =
        titleH + rowHeights.reduce((a, b) => a + b, 0) + (TIERS.length + 1) * 2;
  ...
  const renderCard = (item: TierItem, zone: ZoneId) => {
    ...
    return (
      <div
        ...
        className={`relative shrink-0 cursor-grab touch-none select-none border border-border bg-background overflow-hidden active:cursor-grabbing ${
  ...
  return (
    <main className="w-full max-w-4xl mx-auto px-4 py-6 space-y-5">
      ...
      <input
        ...
        aria-label="标题（可自定义）"
        spellCheck={false}
        className="w-full bg-transparent text-center text-3xl sm:text-4xl font-bold outline-none border-b border-transparent hover:border-border/50 focus:border-border py-2 transition-colors"
      />
        ...
        {TIERS.map((t, i) => {
          ...
          return (
            ...
            >
              {/* 标签块 */}
              <div
                className="flex w-16 shrink-0 items-center justify-center px-1 text-center font-bold leading-tight text-[#171717]"
                style={{
                  backgroundColor: t.color,
                  fontSize: t.label.length >= 3 ? '1.05rem' : t.label.length === 2 ? '1.5rem' : '1.875rem',
                }}
              >
                {t.label}
              </div>
              ...
              <div
                ...
                className={`flex min-h-[88px] flex-1 flex-wrap content-start gap-1.5 p-1.5 transition-colors ${
                  fileOver === t.id ? 'bg-foreground/10 ring-2 ring-inset ring-primary' : isOver ? 'bg-foreground/10' : 'bg-background/40'
                }`}
              >
      ...
      <div>
        ...
        <div
          ...
          className={`flex min-h-[96px] flex-wrap content-start gap-1.5 border border-dashed p-2 transition-colors ${
            fileOver === 'pool'
              ? 'border-primary bg-foreground/10 ring-2 ring-inset ring-primary'
              : drag?.over === 'pool'
