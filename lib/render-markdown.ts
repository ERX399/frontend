...
/* 站外链接在新标签打开。博客/论坛/WebNovel 编辑器预览共用这一份渲染管线，
   一处改动三处同时生效。判据是「绝对 http(s) URL」：相对链接（站内 /xxx、
   锚点 #xxx、mailto:/tel: 等）保持原窗口。
   必须同时带 rel="noopener noreferrer"：不加的话新标签页里的站点能通过
   window.opener 拿到本页的 window 引用（tabnabbing）。 */
...
/* ── :::type 容器警示（markdown 原生警示语法，markdown-it-container@4 的等价实现）──
   markdown-it 核心不认 `:::` 容器，上面那套只认得 `> [!type]` 引用警示。
   这里在 block ruler 加一条规则，把
     :::caution [可选标题]
     …正文（仍是 markdown，可嵌套）…
     :::
   渲染成与 `> [!caution]` **逐字节一致**的 .callout 结构（复用 CALLOUT_ICONS/TITLES），
   所以博客与论坛、服务端与客户端（renderMarkdown 是唯一入口）自动同时生效。
   规则要点（对照官方插件源码，别凭印象改）：
   - marker 是单字符 `:`，至少 3 个；`params.trim()` 让 `:::caution`（无空格）也识别
   - 闭合行 = 冒号数不少于开头 + 尾巴仅空白 → 内层 `:::caution` 这种开标记不会误当闭合，
     嵌套容器才成立；缩进 ≥4 的行是代码，不算闭合
   - 未闭合按 markdown-it 惯例自动闭到父块/文档末尾（和未闭合的围栏代码块一致） */
const CONTAINER_MIN_MARKERS = 3;

const containerBlockRule = (state: any, startLine: number, endLine: number, silent: boolean): boolean => {
  let pos: number;
  ...
  for (pos = start + 1; pos <= max; pos++) {
    if (state.src.charCodeAt(pos) !== 0x3a) break;
  }
  const markerCount = pos - start;
  if (markerCount < CONTAINER_MIN_MARKERS) return false;

  const markup = state.src.slice(start, pos);
  const params = state.src.slice(pos, max).trim();

  // 第一段必须是已知警示类型（TIP/INFO/NOTE/WARNING/CAUTION/DANGER/IMPORTANT，大小写不敏感）
  const type = params.split(/\s+/, 1)[0].toLowerCase();
  if (!CALLOUT_TYPES.includes(type.toUpperCase())) return false;
  ...
  for (;;) {
    nextLine++;
    if (nextLine >= endLine) break; // 未闭合 → 自动闭到父块/文档末尾
    start = state.bMarks[nextLine] + state.tShift[nextLine];
    max = state.eMarks[nextLine];
    if (start < max && state.sCount[nextLine] < state.blkIndent) break; // 负缩进非空行终止容器
    if (state.src.charCodeAt(start) !== 0x3a) continue;
    if (state.sCount[nextLine] - state.blkIndent >= 4) continue; // 缩进≥4 是代码块，不是闭合
    let p = start;
    while (p < max && state.src.charCodeAt(p) === 0x3a) p++;
    if (p - start < markerCount) continue; // 闭合冒号不能少于开头
    p = state.skipSpaces(p);
    if (p < max) continue; // 尾巴只能有空白
    autoClosed = true;
    break;
  }
  ...
  const openToken = state.push('container_admonition_open', 'div', 1);
  ...
  const closeToken = state.push('container_admonition_close', 'div', -1);
...
md.block.ruler.before('fence', 'admonition', containerBlockRule, {
  alt: ['paragraph', 'reference', 'blockquote', 'list'],
});

md.renderer.rules.container_admonition_open = (tokens, idx) => {
  const type = tokens[idx].info.split(/\s+/, 1)[0].toLowerCase();
  // 可选标题：`:::warning 自定义标题`；缺省用 CALLOUT_TITLES 的默认名。
  // 标题是用户内容，必须 escapeHtml（`> [!type]` 那套标题来自固定表，无需转义）
  const title =
    tokens[idx].info.replace(/^\S+\s*/, '') || CALLOUT_TITLES[type] || type.toUpperCase();
  return `<div class="callout callout-${type}"><div class="callout-title">${CALLOUT_ICONS[type] ?? ''}${escapeHtml(title)}</div>`;
};
md.renderer.rules.container_admonition_close = () => '</div>';
...
export function addHeadingIds(html: string): string {
  // 先收集显式写死的 id（markdown 里 html:true 可以直接嵌 HTML），生成新 id 时绕开。
  const explicit = new Set<string>();
  for (const m of html.matchAll(/<(h[1-6])\b([^>]*)>/gi)) {
    const id = m[2].match(/id="([^"]+)"/);
    if (id) explicit.add(id[1]);
  }
  // 同名标题必须分到不同锚点：首次出现保留原名（老外链 `#xxx` 不断），重复的按
  // -1/-2 往后排。否则两个标题同 id，TOC 点了全跳第一个、scroll-spy 全高亮同一条。
  ...
  return html.replace(
    ...
    (_, tag, attrs, text) => {
      ...
      const base = text
        ...
        .replace(/^-+|-+$/g, '');
      // 纯符号/emoji 标题生成空 id，两种 TOC 都按空 id 跳过，不给它编号
      if (!base) return `<${tag}${attrs} id="">${text}</${tag}>`;
      let n = seen.get(base) ?? 0;
      let candidate = n === 0 ? base : `${base}-${n}`;
      while (explicit.has(candidate)) {
        n++;
        candidate = `${base}-${n}`;
      }
      seen.set(base, n + 1);
      return `<${tag}${attrs} id="${candidate}">${text}</${tag}>`;
    },
  );
}
