 ...
 * `/forum/post/:id` 上最大的一个 chunk，再加上 DOMPurify —— 而这两样首屏一个
...
export function loadRenderer(): Promise<Renderer> {
  ...
  loading ??= Promise.all([
    ...
    // target 要显式放行：DOMPurify 默认白名单没有它，renderMarkdown 给站外链接加的 target="_blank" 会被剥掉
    cached = (markdown: string) => purify.sanitize(md.renderMarkdown(markdown || ''), { ADD_ATTR: ['target'] });
    return cached;
  });
  return loading;
}
