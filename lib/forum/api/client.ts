...
export function getBaseUrl(): string {
  ...
  try {
    return localStorage.getItem('forum-api-base-url') || FORUM_API_BASE_URLS.prod;
  } catch {
...
const DEVICE_FINGERPRINT_KEY = 'forum-device-fingerprint';

/**
 * 每台浏览器一个持久指纹（localStorage，按 origin 隔离）。登录时随
 * `X-Device-Fingerprint` 头发给后端，后端据此把同一浏览器的重复登录合并成
 * 同一台设备，而不是每次登录都增生一条会话（个人中心「登录设备」无限增生）。
 * SSR 侧没有 window，返回 null（不发送该头）。
 */
function getDeviceFingerprint(): string | null {
  ...
  try {
    ...
    if (!fp) {
      // crypto.randomUUID 不可用时退回时间戳+随机数拼的短串（同样满足后端的格式校验）
      fp =
        crypto.randomUUID?.() ||
...
async function forumRequest<T>(
  ...
  // 设备指纹随每个请求走（后端只在登录时用，多带无害）。
  // startGithubOAuth 也走这里 → 指纹进了 OAuth state，跳出去再跳回来仍在
...
export async function uploadFile(file: File, type: 'avatar' | 'post' = 'avatar') {
  ...
  const baseUrl = localStorage.getItem('forum-api-base-url') || 'https://i.2x.nz';
